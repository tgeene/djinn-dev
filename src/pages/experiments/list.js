	...
	const link_list = [
		{
			...
			'description': "This tool allows you to select from a few different options to generate unique and secure passwords based on your needs."
		},
		{
			'url':         "/ipsum-generator",
			...
			<nav>
				<ul>
					{ link_list.map((item, i) => (
						<li key={ i }>
							<Link to={ "/experiments" + item.url }
								  title={ item.text + " | Djinn Development" }>{ item.text }</Link> - { item.description }
						</li>
					)) }
				</ul>
			</nav>
